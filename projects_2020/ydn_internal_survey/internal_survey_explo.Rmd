...
(race_plot <- race_filtered %>%
  ...
  coord_polar("y", start=0) + 
  theme_void() +
  geom_label(aes(y = ypos, x = 1.7, label = sprintf("%.1f%%", prop)), color = "white", size=4, show.legend = FALSE) +
  theme(legend.title = element_blank()) +
  scale_fill_brewer(palette = "Paired") +
  ggtitle("Race/Ethnicity Breakdown") +
  theme(plot.title = element_text(hjust = 0.5)))
...
```{r}
grid.arrange(race_plot, gender_plot, nrow = 1, ncol = 2)
```
...
st <- d %>%
  ...
  coord_polar("y", start=0) + 
  theme_void() +
  geom_text(aes(x = 1.24, y = ypos, label = num), color = "white", size=6) +
  theme(legend.title = element_blank()) +
  scale_fill_brewer(palette = "Paired") +
  ggtitle("Highschool Type") +
  theme(plot.title = element_text(hjust = 0.5))

grid.arrange(ht, st, nrow = 1, ncol = 2)
...
# Putting plots side by side
grid.arrange(loafall_plot, loaspring_plot, innewhaven_plot, nrow = 1, ncol = 3)
```
